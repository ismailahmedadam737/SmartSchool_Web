...
class _AdminMessagesPageState extends State<AdminMessagesPage> {
  ...
  Future<void> _fetchMessages() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final List<Map<String, dynamic>> msgs = List<Map<String, dynamic>>.from(json.decode(response.body));
        setState(() {
          AdminMessagesPage.globalMessages = msgs;
        });
        AdminMessagesPage.seenMessageCount = msgs.length;
  ...
  // Shaqadan ayaa tirtiraysa xogta database-ka iyo liiska UI-ga
  Future<void> _deleteMessage(int id, int index) async {
    try {
      final response = await http.delete(
        Uri.parse('https://smartschool-web.onrender.com/api/communications/$id'),
      );
      if (response.statusCode == 200) {
        setState(() {
          AdminMessagesPage.globalMessages.removeAt(index);
        });
        if (AdminMessagesPage.seenMessageCount > 0) {
          AdminMessagesPage.seenMessageCount--;
        }
        AdminMessagesPage.unreadCountNotifier.value = 0;
      }
    } catch (e) {
      debugPrint("Khalad tirtirid: $e");
    }
  }
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        IconButton(
                                          ...
                                          onPressed: () {
                                            // Waxaan wacayaa shaqada tirtiridda
                                            _deleteMessage(msg['id'], index);
                                          },
                                        ),
                                        const SizedBox(width: 10),
