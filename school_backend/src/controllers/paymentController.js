...
// 4. Shaqada cusub ee Reset All-ka
const resetAllPayments = async (req, res) => {
    try {
        await Payment.resetAllPayments();
        res.json({ message: "Dhammaan xogta lacagaha waa la tirtiray oo waa la reset gareeyay!" });
    } catch (err) {
        console.error("DEBUG: Cilad ka dhacday Reset All:", err.message);
        res.status(500).json({ error: err.message });
    }
};
